...
{
    public class OPMRandom : IOPMRandom
    {
        private static Random _random = new System.Random();//随机发生器
        private static char[] _randomlibrary = new char[0];//随机库

        public OPMRandom(OPMConfig config)
        {
            _randomlibrary = config.Randomlib.ToCharArray();
        }

        /// <summary>
        /// 随机库
        /// </summary>
        public static char[] RandomLibrary
        {
        ...
        public string CreateRandomValueWithoutZero(int length, bool onlyNumber)
        {
            int index;
            StringBuilder randomValue = new StringBuilder();
            ...
            {
                if (onlyNumber)
                    index = _random.Next(1, 9);
                else
                    index = _random.Next(1, _randomlibrary.Length);
